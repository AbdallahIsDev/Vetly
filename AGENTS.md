...
**All four dropdown surfaces (single listbox, multi listbox, phone dialog, calendar export menu) enter with `opacity 0→1 + 4px rise, 0.14s easeOut` and exit with the mirrored fade; the export menu rises from its open side.** Each portal is persistent with a menu-scoped `AnimatePresence` *inside* it around a keyed `motion.ul`/`motion.div` — never the portal as the direct `AnimatePresence` child (that shape broke all menus in the Framer runtime, BE-123 follow-up); documented fallback is enter-only motion with instant close. Rect measurement, reposition logic, portal targets, and focus contracts (trigger-focus for select/multi/phone, real-focus menuitems for export, synchronous Escape/outside/blur close) are untouched; first render stays closed so hydration parity holds. This is sanctioned alongside the footer-label `AnimatePresence` (rule 22) — step containers stay banned (rule 21).
