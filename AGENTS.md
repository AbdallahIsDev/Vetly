...
**The interactive gate (`useBeInteractive`) is ASYMMETRIC across the two renders that matter: the headless prerender captures `false`, while a real visitor's first render observes `true` (the module microtask flips `BE_INTERACTIVE` before React hydrates). Therefore: (1) NO initial state may be derived from the gate's value — initial state must be a pure function of config/props; (2) every effect that can commit state reachable by the first paint must check the gate BEFORE its first commit and commit NOTHING while it is `false`; (3) `useState(BE_INTERACTIVE)` is intentional and must not be "fixed" to always-false — real visitors need gated effects to run in the first pre-paint pass.**

This refines the PRERENDER-DEFER architecture (rules 42/105). Framer's published-site prerender runs component effects inside a headless Chrome, and any state it commits is BAKED into the served HTML — which no visitor's first render can reproduce, producing the #425/#418/#422 recoverable-error family on every load. Concrete invariants that must never regress:

- **`detectAutomationPrerender`** must stay hardened: `navigator.webdriver === true` OR a `HeadlessChrome` UA OR the `https://screenshot.framer.invalid` capture origin. A missed detection keeps the gate open during capture.
...
- **`useCalcomSlots`**: the fetch effect checks the gate FIRST — the prerender must never reach the `setLoading(false)` early-return branches (the initial `loading=true` is server-identical).
...
- **sessionStorage restore + persist** are gate-deferred: a warm prerender container must never restore a step into (or write/delete storage from) the served HTML. Real visitors still get the pre-paint restore in the same pass the gate flips — rules 7/16/20/74 unchanged.
