...
**`Buttons > Button Texts` exposes exactly Next Step / Back Step / Final Action (title-only renames; stored keys `continueLabel`/`backLabel`/`finalActionLabel`, values "Continue"/"Back"/"Book Now").** Cancel and Retry render their fixed strings with no control, interface key, or legacy carrier — the `buttonTexts.cancelSubmitLabel`/`retryLabel` keys, the flat `cancelSubmitLabel`, and the `copy.retryLabel` carrier were deleted; stored custom Cancel/Retry values intentionally froze to "Cancel"/"Try again" (recorded on the BE-038 entry). Do not re-add rows or carriers for them. (BE-038)
