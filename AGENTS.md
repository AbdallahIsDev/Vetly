...
**State flips fade instead of snapping (batch 1, BE-124):** choice selected-ring (`box-shadow`), segmented option color, radio-dot scale/opacity (always-rendered), today-dot scale/opacity (always-rendered), month-nav buttons, slot elapsed opacity, phone/export row color, input + phone-group focus rings (constant CSS, covered by the global reduced-motion query), Booking spinner mount, multiselect chip mount, phone globe/flag + dial swaps, field-error mount, slots-error banner mount, success detail stagger + action-row fade. Every one is reduced-motion gated (JS gates or `MotionConfig reducedMotion="user"` + the global `prefers-reduced-motion` query). Do not animate cards-grid reflow or calendar geometry (pure-CSS no-measurement architecture), mask fades (rule 152), closed-box value text, or submit disabled/aria timing.
