...
### 115. Calendar Tiles own the tile surface; Shadow is always last; navigation spans the full runtime pipeline (TILES-SHADOW-CEILING)

**The Calendar panel item owns Title, Subtitle, and a Tiles set - never a Layout control.** The Calendar stage is always single-column by product rule; Form Steps keep their own Single/Two-Column control. The Tiles set is Background, Text Color, Border, Radius, Padding, BG Blur, Shadow (same factories and effective-default vocabulary as field/button Styles): text color drives the surface container plus derived muted tones (on-accent surfaces stay exempt per rule 70); the border mirrors the native `1px solid` Border-token surface (explicit width 0 removes it); tile type itself stays Body-inheritance plus the per-element treatments of rules 55/56/72 (no tile Font row - a compound control cannot target family-only without dead size/variant rows, and global Body Font already flows in). Legacy marker surfaces feed the same keys. Never re-add a Calendar Layout control, a second placement mechanism, or tile controls outside this vocabulary.

**Shadow is the final row in every Styles submenu that carries one** - field input/choice/checkbox sets, Calendar Tiles, button groups (BG Blur, then Hover/Pressed, then Shadow), and interaction states (already final). This supersedes rule 101's old order sentence. Do not append Gap/spacing, selected-state, decor, or any new row after Shadow; new rows go before it.
