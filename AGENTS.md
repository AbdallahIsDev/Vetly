...
### 115. Calendar Styles own the tile surface; Shadow is always last; navigation spans the full runtime pipeline (TILES-SHADOW-CEILING)
...
**Shadow is the final row in every Styles submenu that carries one** - field input/choice/checkbox sets, Calendar Styles, button groups (BG Blur, then Hover/Pressed, then Shadow), and interaction states (already final). This supersedes rule 101's old order sentence. Do not append Gap/spacing, selected-state, decor, or any new row after Shadow; new rows go before it.
...
**Calendar tile type uses the shared Font row, not a parallel system.** The Calendar Styles set owns one `Font` row (`fieldStylesFontControl`, 14px Regular effective default) driving date-number family/size/line-height; weight is never applied (date numbers stay 500 in every state, rule 55). It renders only on date numbers — weekday labels (12px/700), month title, and time slots keep their own specs. Unset renders byte-identically to before (inherit/14px/inherit). Shadow stays the final row in the Calendar set, per the SHADOW-LAST rule.
...
**Layout controls never reorder DOM.** Justification/alignment move boxes visually; Back always precedes the primary action in the DOM because that order IS the keyboard tab order — visual-vs-DOM order mismatch (row-reverse, flex `order`, or a "primary left / Back right" option) is an accessibility violation, not a design choice. The same holds for title-before-subtitle and grid/column structures: alignment is customizable, sequence is not.

**Concrete applications:** the `Header` group owns step title+subtitle `Alignment` (Left/Center/Right, unset = historical inherit); success/error terminal states keep their designed centered alignment and are out of its scope. `Grouped Alignment` (Left/Center/Right, default Right) positions the grouped nav row and hides in Split mode; Split (space-between) and single-button (flex-end) justification are definitional and have no control. Never add spacing/width/position rows for footer rhythm (`gap 8`, `marginTop 24`, sticky), calendar geometry (7-column grid, 1:2:1 tracks, slot column), or progress structure (full-width track) — those are internal rhythm, not design decisions. The be-layout-controls-matrix harness (options/order/defaults, default-identical mapping, DOM-order preservation) passes before merge.
...
### 125. Header alignment is a per-authored-Step property, never a global Booking Engine property (HEADER-ALIGN-PER-STEP)

**Header alignment (step Title + Subtitle text alignment) belongs to each authored Step, not to the component.** The Alignment control (Left / Center / Right) lives inside every Step's own submenu — Visible → Title → Subtitle → Alignment → Layout → Fields — and each Step resolves its value independently: changing Step 1's alignment must never affect Step 2 or any other Step. Do not reintroduce a top-level `Header` Property Control group (the group and its global `Alignment` control were removed on purpose); do not add a second alignment control anywhere else; do not centralize resolution back into a single component-wide value. Do not re-add the group "unless more global header controls appear later" — if genuinely global header controls are ever introduced, that is a new author-direction decision, not a reason to restore this one.

**Resolution and migration contract:** `StepConfig.alignment` (optional; the `...step` spread carries it verbatim through `migrateLegacyCalendar` and `normalizeSteps` — never normalize/coerce it there, the render site must keep distinguishing "never authored" from explicit Left). Resolution happens per step at the render site via `isStepAlignment`; unset/junk renders exactly the historical inherited-left look, so untouched canvases are byte-identical. The stored global value survives as a **read-only legacy carrier** (`header?.alignment`, interface key with no control): `useBookingEngineState` reads it at exactly ONE site and seeds only steps that never authored their own alignment, so pre-move canvases keep their authored look. The config fingerprint (rule 106) deliberately excludes `alignment` — visual-only edits never rekey autosave. Success/error terminal states keep their designed centered alignment and are out of scope for this control.
