...
**Inside every field's Styles submenu, `Label Font` is listed first and `Font` second; the last numeric row is titled `Gap` (the `spacing` key is unchanged — title only).** Do not reorder them back and do not rename `Gap` to `Spacing`. In the input set, `Focus Border` sits with the post-five box rows (after `Padding`), keeping the shared Fill → Radius → Border → Shadow → BG Blur run consecutive (rule 130).
...
**The Calendar panel item owns Title, Subtitle, and a Styles set - never a Layout control.** The Calendar stage is always single-column by product rule; Form Steps keep their own Single/Two-Column control. The Styles set is Background, Text Color, Border, Radius, Padding, BG Blur, Shadow (same factories and effective-default vocabulary as field/button Styles): text color drives the surface container plus derived muted tones (on-accent surfaces stay exempt per rule 70); the border mirrors the native `1px solid` Border-token surface (explicit width 0 removes it); tile type itself stays Body-inheritance plus the per-element treatments of rules 55/56/72 (no tile Font row - a compound control cannot target family-only without dead size/variant rows, and global Body Font already flows in). Legacy marker surfaces feed the same keys. Never re-add a Calendar Layout control, a second placement mechanism, or tile controls outside this vocabulary.

**The canonical base order is Fill/Background → Radius → Border → Shadow → BG Blur, consecutive, in every Styles submenu that carries those rows** — field input/choice/checkbox sets (whichever apply; never add inapplicable rows), Calendar Styles, and button groups (where content `Text` stays first per rule 99, then Typography, then the five, then `Padding`, then Hover/Pressed). This supersedes the old absolute-last Shadow rule: Shadow stays consecutive inside the five, BG Blur follows it, set-specific rows (Padding, Focus Border, Gap, Selected-*, Size) follow BG Blur, and interaction states close button groups. Runtime layering is untouched (shadow composes under state rings). Do not reorder the five back and do not interleave set-specific rows into them; new rows go after BG Blur (before states, if any).
...
**Calendar tile type uses the shared Font row, not a parallel system.** The Calendar Styles set owns one `Font` row (`fieldStylesFontControl`, 14px Regular effective default) driving date-number family/size/line-height; weight is never applied (date numbers stay 500 in every state, rule 55). It renders only on date numbers — weekday labels (12px/700), month title, and time slots keep their own specs. Unset renders byte-identically to before (inherit/14px/inherit). The Calendar set follows the canonical order (rule 130): Font, Text Color, Background, Radius, Border, Shadow, BG Blur, Padding.
...
**Content/header alignment is one global component decision, not a per-Step repetition.** The `Content Alignment` control (Left / Center / Right, default Left) lives in the `Content` group and owns step Title + Subtitle text alignment AND success/error terminal content (icon row, titles, subtitles) together — authors configure alignment once. Do not reintroduce a per-step Alignment control; do not add a second global alignment control anywhere else; do not centralize step resolution into anything but this control plus the legacy carriers below.
...
**The control's title is `Buttons Alignment`** (inside `Buttons > Buttons Layout`). It was renamed from "Grouped Alignment" — label-only; options (Left/Center/Right), default (Right), and the conditional visibility (hidden unless `Layout = Grouped`) are unchanged. Do not rename it back, and do not change its gating to any other condition.
...
**Five opt-in controls, all default-identical — as superseded by rule 129:** the pass-2 `Header > Terminal Alignment` (default Center) is replaced by the global `Content > Content Alignment` (default Left; see the rule-129 default tradeoff). Unchanged survivors: input field Styles `Text Align` (unset = inherit; text/email/phone/textarea only), `Buttons > Order` (Back First default, true DOM order — now inside `Buttons Layout`), `Buttons > Width` (Hug default, flex-share fill — now inside `Buttons Layout`). The pass-2 `Layout > Content Width` cap was removed entirely (rule 129) — no control, no runtime branch. No footer spacing rows, no calendar-geometry rows, no label-position rows.

**Mechanism invariants.** New visual keys (`terminalAlignment`, `textAlign`, `buttonOrder`, `buttonWidth`, `contentWidth`) are excluded from the config fingerprint (rule 106) — visual-only edits never rekey autosave and never disturb restore. Text Align applies only when set (`...(fs?.textAlign ? … : {})`), so untouched fields inherit byte-identically (including RTL/page inheritance). Order renders `primaryGroupEl`/`backButtonEl` element consts in configured sequence — single definitions, never duplicated markup, never CSS order/row-reverse. Fill uses flex shares (`1 1 0` + `minWidth: 0`), never pixel math. The Compact cap is a plain max-width wrapper — sticky footer, Calendar placement, and responsive stacking are untouched. The be-layout-controls-matrix harness covers options/order/defaults, default-identical mapping, and DOM-order preservation.
...
**Terminal screens are intentionally customizable — compactly.** Beyond alignment: `Terminal > Icon Size` (24–96px, unset = 64 success / 40 error historical; glyph scales with the mark) is the one terminal sizing control — terminal-scoped, so it lives in the `Terminal` group, never in `Content`. No terminal surface/typography/spacing controls were added because the theme tokens (surface/border/radius), Head Font, and Copy already drive those surfaces — a second control layer would duplicate live customization, not add it. Icon placement follows alignment by construction. Confirmation actions stay right-grouped (rule 31); the error action row keeps its own centering.
...
**`Layout` hides while `Width = Fill` because it has no author-facing meaning there.** Fill gives both buttons equal shares either way, so Grouped/Split positioning is visually moot; the saved Layout value is preserved (hidden never deletes) and restored when Width returns to Hug. While Fill, runtime is deterministic (fill shares + wrap backstop) and never exposes contradictory semantics.
...
**Canonical order — Typography first, then the consecutive base five, then set-specific rows, then interaction states.** Every Styles submenu that carries typography rows lists them first (`Label Font`, then `Font`, with text colors alongside); then the shared base primitives consecutively in exactly this order: Fill/Background → Radius → Border → Shadow → BG Blur. Set-specific rows that are not shared primitives (Padding, Focus Border, Gap, Size, Selected-*, option text) follow the five in stable order; interaction-state subgroups (Hover → Pressed) close button groups. Never add a shared primitive outside its canonical slot, never interleave set-specific rows into the five, and never append rows after interaction states. Checkbox correctly carries no Fill/Radius/Border rows (native box — inapplicable rows are never added). This supersedes the old absolute-last Shadow rule (rules 101/115): Shadow stays consecutive inside the five and runtime layering is untouched (shadow composes under state rings).
...
**Property Controls must live in groups that match the actual scope of the setting.** `Content` holds only the global Content Alignment. `Terminal` holds terminal-specific Icon Size (moved out of Content; the old `header.iconSize` stays readable as fallback). `Styles` holds global visual tokens. `Field Styles` holds shared field defaults. `Buttons` holds nav layout + per-button groups. Do not group unrelated controls merely because their names both contain "Content" or "Styles." The config fingerprint excludes all of these visual keys — visual-only edits never rekey autosave.
