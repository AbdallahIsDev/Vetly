...
This is a **non-configurable core behavior**. It must not be removed or weakened by future optimization, refactoring, animation changes, or review passes. The form data must be kept in stable parent-level state that survives step unmounts/remounts. Never rely on the step component lifecycle for state preservation, and never reset the state upon entering a step. **The one exception is a field-type change: field ids are positional, so changing a field's Type orphans its stored value (a select seed lingering in a field that is now text) - the first render with the new type drops that field's value and error exactly once, treating it as brand-new (BE-116). Config (including the placeholder) is never touched and always survives type changes.**
...
**There is no Height row in any Styles submenu.** Field height is hardcoded to a `23px` min-height floor at the consumption sites (inputs, textarea, choice options) and grows via Padding only — to make a field taller, authors increase Padding. A `minHeight` stored by an older canvas is still honored as legacy (never silently restyle a saved instance), but no control exposes it. Empty select/multiselect triggers render a non-breaking-space strut (never a plain space, which collapses) so their height always matches populated fields - a content-less element has no line box and floors at the 23px minimum (BE-112, proven in-browser); this is not a select placeholder (rule 133 stands). Do not re-add a Height control; do not raise the floor back to the 44px touch target (buttons keep 32px, calendar cells and the segmented control keep their own minima — this rule covers form-field/option surfaces only).
...
**Head + Body fonts lead the Styles panel inside the Header subgroup (`styles.header.headingFont` / `styles.header.font`) — there is no Font group.** Typography leads (Head first, Body second, then color tokens and metrics); the retired top-level `typography` group stays readable as the middle fallback (`styles.header.font ?? styles.font ?? typography?.font ?? props.font`, same for heading). Font defaults (22px Bold head, 15px Regular body) are unchanged. Do not re-add a top-level Font group, do not reorder fonts below the color tokens, and do not read raw `props.font` / `props.headingFont` anywhere except the resolution lines. (Amended by BE-118, 2026-09-11: Text Align + Head Font + Body Font grouped into one `Header` item opening a submenu — nesting only, flat keys stay as legacy carriers.)
...
**Content/header alignment is one global component decision, not a per-Step repetition.** The `Content Alignment` control (Left / Center / Right, default Left) is the FIRST row inside the `Styles > Header` subgroup (amended by BE-044 — the emptied `Content` group is gone; amended by BE-118 — Text Align + Head Font + Body Font grouped under one `Header` item; the stored path is `styles.header.contentAlignment` with `styles.contentAlignment`/`header.contentAlignment`/`header.terminalAlignment` as readable legacy carriers) and owns step Title + Subtitle text alignment AND success/error terminal content (icon row, titles, subtitles) together — authors configure alignment once. Do not reintroduce a per-step Alignment control; do not add a second global alignment control anywhere else; do not centralize step resolution into anything but this control plus the legacy carriers below.
...
**`Content Alignment` (Left/Center/Right, default Left) is the FIRST row inside the `Styles > Header` subgroup — there is no `Content` control group.** The stored path is `styles.header.contentAlignment`; the previous `styles.contentAlignment`, `header.contentAlignment` and `header.terminalAlignment` values keep applying as readable legacy carriers at one resolution site, so a saved canvas renders identically after the move. Scope and behavior are unchanged (step headers + terminal headers together, per-step carriers, Calendar excluded, never merged with Buttons Alignment — rule 125's amended clauses stand). Do not re-create a one-row Content group and do not drop the legacy reads. (BE-044)
...
**`multiselect` is its own field type, never a toggle on `select`.** `MultiSelectFieldControl` mirrors the single-select combobox mechanics (trigger + portaled listbox, `aria-activedescendant`, focus never leaves the trigger, outside/Escape close, scroll/resize reposition) with toggle-in-place rows (accent-colored check as the sole selected mark - rows never change background for selection, hover wash is hover-only, BE-114), removable chips (author-supplied lucide X glyph), empty placeholder, per-value hidden inputs, and `aria-multiselectable`. There is intentionally NO first-option seed and no Default Selected row for this type — unset renders zero chips. `BookingValues` holds `string[]` for these fields; `isEmptyPayloadValue`/`validateField` treat `[]` as empty (required = >=1 picked); the Cal.com payload sends the raw array under the slug (never joined text); notes/success join with ", " for display only. `calTypeToFieldType` maps `multiselect` here; `selectgroup` stays `select` (single-valued). Do not merge this into `select`, do not seed it, do not join it. (BE-055)
...
**Every `phone` field renders `PhoneFieldControl`: a flag + unfold-indicator button, a muted dial-text middle slot, and the national-number input — three flush parts with no gaps (the attached reui `phone-input.tsx` UI contract, reimplemented dependency-free in plain inline styles; the reference file is never imported). (no gap, shared border, split radii ΓÇö the attached reui `phone-input.tsx` UI contract, reimplemented dependency-free in plain inline styles; the reference file is never imported).** The indicator is the author-supplied unfold-more glyph rendered at 16px with BOTH chevrons filled solid (the source file's upper chevron was stroke-only), muted and fixed-size ΓÇö a pure clickable affordance that never shifts layout. Stored value is always full-international (`+{dial}{digits}` compact; dial-only never stored; empty national stores `""` so required/empty semantics are unchanged). **Country source chain:** deterministic `US` default renders both sides first (rule 42) ΓåÆ one-shot detection, interaction-gated per rule 109 (explicit non-default locale region wins first; otherwise the device timezone resolves the ambiguous remainder via `PHONE_TIMEZONE_TO_ISO` ΓÇö e.g. en-US + Africa/Cairo ΓåÆ EG ΓÇö else US; no IP geolocation ever) ΓåÆ stored-value parse on restore (current-country-wins ties on shared dials +1/+7/+44, canonical representatives US/RU/GB for foreign fallbacks) ΓåÆ manual pick always wins and is never clobbered. **No second styling system** (shared Field Styles + Selected Styles vocabularies, select/calendar-menu portal mechanics mirrored). **Flags are real images, not emoji** ΓÇö Windows ships no flag-emoji font and renders the bare letter pair (the reported "DZ" box), so `PhoneFlag` renders a flagcdn SVG image inside a fixed 22├ù16 slot and fails closed to a styled two-letter badge when offline or unknown (slot geometry never shifts, so country changes cause no layout shift). **Focus follows the other fields, not buttons:** the trigger carries no `be-input` class (that stacked the inset input ring with the global button outline into the reported odd double-ring) ΓÇö keyboard focus shows the same inset 2px ring in the focus color that inputs show, with the global button outline suppressed as replaced, raised above the input edge. **The seam is adjacency, never overlap:** the trigger carries no right border and the input keeps its left border, sitting exactly side by side ΓÇö one 1px divider in every state (an overlap attempt briefly hid the divider under the positioned trigger and was reverted); the trigger focus ring is error-aware (`hasError` ΓåÆ error color, mirroring `.be-input-invalid:focus-visible`). **Focus rings are joint-aware (BE-103):** whichever half holds keyboard focus paints its ring on three sides only ΓÇö trigger omits the right edge, input omits the left ΓÇö so the seam divider never doubles under focus while the joined control keeps one continuous ring in the exact color other fields use. **Dropdown interaction contract:** the outside-close root is the whole dialog (search + list ΓÇö rooting it on the list alone closed the menu on search/padding presses); the country list takes the shared `be-select-scroll` hidden-scrollbar treatment exactly like the select menus (author order BE-088 ΓÇö scroll via wheel/touch/arrows); the search row is icon + borderless input with no ring (autofocus retained so typing works on open); rows are flag + name + dial-at-far-right with no check glyph (accent selected surface is the indicator); empty search renders "No country found.". **The national box placeholder defaults to the derived `+{dial}`** ΓÇö an author-configured phone placeholder wins when set (BE-098 restores BE-096 after BE-097's hide-the-row went too far ΓÇö freedom over restriction). Existing `validatePhone` (format + ΓëÑ7 digits), write-point sanitization (rule 97, `+` owned by the button), max-40, notes/success display, and the hidden-input name transport all ride unchanged underneath. (BE-063, UI amended by  **The dial slot is a fixed 3em border-box shared by display and dial-edit modes, so `+1` vs `+224` never shifts the layout (BE-106).** A bare `+` alone — empty box or select-all replacement — always opens dial-edit mode with the number cleared, and the trigger shows the globe while codeless, in the same fixed 22x16 slot as the flag so the swap never shifts layout (BE-111/BE-113).** **Span + input live inside one bordered `be-phone-group` box that owns the border (longhands only, no left edge); the input inside is borderless (its shared shorthand is neutralized via `border: undefined` so error-time color flips cannot resurrect a width — the BE-103 trap mirrored — plus a lone constant `borderWidth: 0`), and group focus arrives via `:focus-within` + an inline ring-color var with zero JS state (BE-108/BE-109).**  **National length is hard-capped per country at the write point, never an error message (BE-104/BE-110): the budget is min(metadataMax, 15 minus dialLength), with metadataMax extracted from libphonenumber possibleLengths per country (Egypt 10, US 10, Germany 13) and the E.164 ceiling as backstop - no routable number on earth exceeds its budget.** (BE-063, UI amended by BE-086/BE-087/BE-088/BE-089/ BE-096/BE-097/BE-098/BE-101/BE-105/BE-106/BE-108/BE-110) **The national input keeps the shared padding on top/right/bottom with `paddingLeft: 0`, so the typed digits sit flush against the dial slot (BE-120).** **The flag trigger takes top/bottom/left from the shared padding axes (`padAxes.y`/`padAxes.x`) while its right padding stays fixed at 8px, so the seam divider never drifts (BE-121).**
...
### 198. Dropdown selected rows are check-only; all menus share one enter/exit pattern; micro-interactions fade (MENU-ANIMATION)

**No dropdown row ever takes an accent/primary background for selection — single-select, multiselect, and export rows alike.** Selected single-select options show the same 16px accent check glyph multiselect uses (accent when selected, transparent otherwise, `color 0.12s ease`); hover wash follows the active row only. Phone country rows keep their accent surface (they are a search-filtered dialog, not a select listbox) with `background-color + color` transitions. Do not reintroduce selected-row accent surfaces on select/multiselect.

**All four dropdown surfaces (single listbox, multi listbox, phone dialog, calendar export menu) enter and exit in the Transition Type family's shape via `surfaceEnterExit(variant, direction)` (BE-125) — the variant def's inactive shape for a travel direction, on a fixed quick timing (menus 0.14s); the export menu's direction follows its open side.** Each portal is persistent with a menu-scoped `AnimatePresence` *inside* it around a keyed `motion.ul`/`motion.div` — never the portal as the direct `AnimatePresence` child (that shape broke all menus in the Framer runtime, BE-123 follow-up); documented fallback is enter-only motion with instant close. `animate` everywhere is the shared `SURFACE_ANIMATE_RESET` (full property reset); `enter` is typed as the scalar `SurfaceEnterShape` because this framer-motion version rejects `TargetAndTransition` in `initial`. The same helper drives the terminal crossfade pair (forward/backward mirrored), calendar grid, time list, event info, progress block, slots banner, field errors, success rows/action, chips, and the error card/action. Micro property-flips (colors, rings, thumb/progress springs, spinner, skeleton pulse, focus rings) and tiny text/icon swaps are not surfaces and keep their own fades. Rect measurement, reposition logic, portal targets, and focus contracts (trigger-focus for select/multi/phone, real-focus menuitems for export, synchronous Escape/outside/blur close) are untouched; first render stays closed so hydration parity holds. This is sanctioned alongside the footer-label `AnimatePresence` (rule 22) — step containers stay banned (rule 21).

### 199. Author customization is capped at runtime — caps are product decisions, never bugs (CUSTOMIZATION-CAPS)

**Framer exposes min/max only on Number (slider/stepper; typed-value enforcement unconfirmed) and nothing cappable on Padding, Font, Border, Shadow, Radius, or Transition — so every bound below is enforced at runtime and must never be removed as "dead code" or "over-restriction":** field padding 0–16px per side (author order 2026-09-11 — stored values above 16 clamp down); button padding Y 4–20 / X 8–32; fonts head 16–40, body 11–20, label 10–24, field/selected 10–28, button/menu/tile 10–24 with a line-height floor of 1 (`normal` untouched); border widths 0–4px (explicit 0 survives as none); selected padding 0–16 with selected radius/border clamps (segmented-thumb math safety); step duration 0–1.5s + delay 0–1s; radius tokens 0–24 per corner (pill-scale ≥999 and % pass through); shadow lengths ±48px; inner Gap 0–24; Check Size 12–32; Hover Scale 0.95–1.05 / Opacity 0.5–1 (panel narrowed to match); Thumb Stiffness 150–600 / Damping 20–60 (panel + `SEGMENTED_MOTION_LIMITS` match). **Render-level guards are permanent too:** cleared copy/validation/manage rows fall back to defaults (never blank gaps), blank field labels render no element, footer buttons and the closed select truncate with ellipsis (+ minWidth 0), a trailing lone Half spans both tracks, multiselect shows 3 chips max with a `+N` overflow chip (hidden inputs still submit everything), and cards/radio labels/descriptions clamp to 2/3 lines. The already-guarded set (Radius token, Gap, section gaps, thumb prefs, counts, validation caps, Cal.com fallbacks, persistence, menu geometry, fixed floors) stays as is. Do not re-widen a range, re-expose a removed freedom, or "fix" a clamp away — an author hitting a cap is the protection working.

**State flips fade instead of snapping (batches 1+2, BE-124):** choice selected-ring (`box-shadow`), segmented option color, radio-dot scale/opacity (always-rendered), today-dot scale/opacity (always-rendered), month-nav buttons, slot elapsed opacity, phone/export row color, input + phone-group focus rings (constant CSS, covered by the global reduced-motion query), Booking spinner mount, multiselect chip mount, phone globe/flag + dial swaps, field-error mount, slots-error banner mount, success detail stagger + action-row fade, terminal crossfade (single-return `AnimatePresence mode="wait"`, opacity only — success/error are elements, not early returns), calendar-grid directional slide + header-text fade (keyed container remount, skeleton instant), time-list swap fade (radiogroup keyed by date+format), event-info status fade, progress-block mount + counter/pct fades, select closed-box value fade, error-screen enter (mark scale + card/action fades). Enter-only animations gate `initial` on the shared `useMountedOnce` hook (first paint byte-identical; reduced motion via tree-wide `MotionConfig` plus local gates). Every one is reduced-motion gated (JS gates or `MotionConfig reducedMotion="user"` + the global `prefers-reduced-motion` query). Do not animate cards-grid reflow or calendar geometry (pure-CSS no-measurement architecture), mask fades (rule 152), chip exit/layout (focus-on-remove risk), or submit disabled/aria timing. All six step variants are direction-aware (`inactive` as `custom` functions — Back reverses Forward for every variant, rule 23).
